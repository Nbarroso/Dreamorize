	...
	$page.hammer()
		...
		.on('drag', function(e){
			...
			if(e.gesture.direction == "right" && !sidebar){
				$page.css({ transitionDuration: '0s', transform: 'translateX(' + e.gesture.deltaX + 'px)'});
			}
			if(e.gesture.direction == "left" && sidebar){
				$page.css({ transitionDuration: '0s', transform: 'translateX(' + 150 + e.gesture.deltaX + 'px)'});
		...
		.on('dragend', function(e){
			if(e.gesture.direction == "right" && !sidebar){
				if(e.gesture.deltaX <= 75){
					//$page.css({transform: 'translateX(0px)'});
					$page.css({ transitionDuration: '0.3s', transform: 'translateX(0px)'})
					sidebar = 0;
				}else{
					$page.css({ transitionDuration: '0.3s', transform: 'translateX(150px)'});
					sidebar = 1;
				}
			}
			if(e.gesture.direction == "left" && sidebar){
